...
<body>
    ...
    <div id="historyModal">
        <div class="history-content">
            <h2 style="color: #4b6cb7; text-align: center; margin-top: 0; font-size: 28px;">LỊCH SỬ PHỤC HỒI CHỨC NĂNG</h2>
            <table class="history-table">
                <thead>
                    <tr>
                        ...
                        <th>Góc gập TB</th>
                        <th>Độ rung TB</th>
                        <th>Đánh giá lâm sàng</th>
                    </tr>
                </thead>
                <tbody id="historyTableBody"></tbody>
            </table>
            <div class="history-actions">
                <button class="btn" onclick="closeHistory()" style="width: auto;">ĐÓNG</button>
                <button class="btn btn-danger" onclick="clearHistory()" style="width: auto;">XÓA DỮ LIỆU</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function startApp() {
            ...
            if (!patientName) { alert("Vui lòng nhập tên bệnh nhân!"); return; }
        ...
        function showHistory() {
            ...
            if (history.length === 0) {
                tbody.innerHTML = '<tr><td colspan="6" style="padding: 30px; color: #888;">Chưa có dữ liệu y án nào được ghi nhận trên thiết bị này.</td></tr>';
                return;
            }
            
            // Hiển thị dữ liệu mới nhất lên đầu
            history.slice().reverse().forEach(record => {
                let diagColor = record.diagnosis.includes("BẤT THƯỜNG") ? "var(--danger)" : "var(--success)";
                tbody.innerHTML += `
                    <tr>
                        <td>${record.date}</td>
                        ...
                        <td style="color: ${diagColor}; font-weight: bold;">${record.diagnosis.replace('PHÁT HIỆN BẤT THƯỜNG: ', '')}</td>
                    </tr>
                `;
            });
        }
        ...
        function clearHistory() {
            if (confirm("Cảnh báo: Thao tác này sẽ xóa toàn bộ lịch sử bệnh án trên trình duyệt hiện tại. Bạn có chắc chắn muốn tiếp tục?")) {
                localStorage.removeItem('bme_patient_history');
                showHistory(); // Refresh bảng
            }
        }
        // -----------------------------
        ...
        function endGame() {
            ...
            const diagMsg = issues.length === 0 ? "Bình thường" : "PHÁT HIỆN BẤT THƯỜNG: " + issues.join(" | ");
